...
using rpc_protocol = rpc::protocol<serializer, RpcVerb>;
...
class messaging_service::rpc_protocol_wrapper {
 ...
 public:
  ...
  auto make_client(RpcVerb t) {
    return _impl.make_client<Func>(t);
  }

  template <typename Func>
  auto register_handler(RpcVerb t, Func&& func) {
    return _impl.register_handler(t, std::forward<Func>(func));
  }

  template <typename Func>
  auto register_handler(RpcVerb t, scheduling_group sg, Func&& func) {
    return _impl.register_handler(t, sg, std::forward<Func>(func));
  }

  future<> unregister_handler(RpcVerb t) { return _impl.unregister_handler(t); }

  void set_logger(::seastar::logger* logger) { _impl.set_logger(logger); }

  bool has_handler(RpcVerb msg_id) { return _impl.has_handler(msg_id); }
...
class messaging_service::rpc_protocol_client_wrapper {
 ...
 public:
  rpc_protocol_client_wrapper(rpc_protocol& proto, rpc::client_options opts,
                              socket_address addr, socket_address local = {})
      : _p(std::make_unique<rpc_protocol::client>(proto, std::move(opts), addr,
                                                  local)) {}

  rpc_protocol_client_wrapper(rpc_protocol& proto, rpc::client_options opts,
                              socket_address addr, socket_address local,
                              shared_ptr<seastar::tls::server_credentials> c)
      : _p(std::make_unique<rpc_protocol::client>(
...
template <typename Func>
void register_handler(messaging_service* ms, RpcVerb verb, Func&& func) {
  ms->rpc()->register_handler(verb, std::move(func));
}

// Send a message for verb
template <typename MsgIn, typename... MsgOut>
auto send_message(messaging_service* ms, RpcVerb verb, msg_addr id,
                  MsgOut&&... msg) {
  auto rpc_handler = ms->rpc()->make_client<MsgIn(MsgOut...)>(verb);
  using futurator = futurize<std::result_of_t<decltype(rpc_handler)(
      rpc_protocol::client&, MsgOut...)>>;
  ...
  return rpc_handler(rpc_client, std::forward<MsgOut>(msg)...)
      .handle_exception([ms = ms->shared_from_this(), id, verb,
...
auto send_message_timeout(messaging_service* ms, RpcVerb verb, msg_addr id,
...
// TODO: Remove duplicated code in send_message
template <typename MsgIn, typename... MsgOut>
auto send_message_cancellable(messaging_service* ms, RpcVerb verb, msg_addr id,
                              abort_source& as, MsgOut&&... msg) {
  auto rpc_handler = ms->rpc()->make_client<MsgIn(MsgOut...)>(verb);
  using futurator = futurize<std::result_of_t<decltype(rpc_handler)(
      rpc_protocol::client&, MsgOut...)>>;
  if (ms->is_shutting_down()) {
...
auto send_message_oneway(messaging_service* ms, RpcVerb verb, msg_addr id,
                         MsgOut&&... msg) {
  return send_message<rpc::no_wait_type>(ms, std::move(verb), std::move(id),
                                         std::forward<MsgOut>(msg)...);
}

// Send one way message for verb
template <typename Timeout, typename... MsgOut>
auto send_message_oneway_timeout(messaging_service* ms, RpcVerb verb,
                                 msg_addr id, Timeout timeout,
                                 MsgOut&&... msg) {
  return send_message_timeout<rpc::no_wait_type>(ms, std::move(verb),
                                                 std::move(id), timeout,
                                                 std::forward<MsgOut>(msg)...);
}
